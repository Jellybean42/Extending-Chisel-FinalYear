...
def normaliseVecFloat (v): #Normalises a vector but returns a float
    norm = numpy.linalg.norm(v)
    if norm == 0:
        return v
    return v / norm

def dotProductVecFloat (normal, pos, offset): #used to check that the plane faces away from the shapes center using the dot product
    return numpy.dot(normal, offset)