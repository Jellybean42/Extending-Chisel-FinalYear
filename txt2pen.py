...
def scanRoom (topleft, p, mapGrid, walls, doors):
    ...
    while True:
        ...
        if (doorStart == None) and lookingLeft (p, leftVec[d], mapGrid, '. '):
            if debugging:
                print("seen first point", p)
            # first point on the wall is a door
            doorStart = addVec (p, leftVec[d])
            doorEnd = doorStart
            doorType = getDoorType (doorStart, mapGrid)
        if lookingLeft (addVec (p, forwardVec[d]), leftVec[d], mapGrid, '. '):
            if debugging:
                print("seen a door point", p, end=' ')
            if doorStart == None:
                doorStart = addVec (addVec (p, forwardVec[d]), leftVec[d])
                doorType = getDoorType (doorStart, mapGrid)
            doorEnd = addVec (addVec (p, forwardVec[d]), leftVec[d])
        else:
            # end of door?
            if doorEnd != None:
                doors += [[doorStart, doorEnd, doorType]]
                doorStart = None
                doorEnd = None
...
def introduceLights (pos, mapGrid, walls, doors):
    global debuging

    s = pos
    a = addVec (pos, [-1, -1])
    d = 1  # 0 is up, then proceeds around clockwise
    leftVec = [[-1, 0], [0, -1], [1, 0], [0, 1]]
    forwardVec = [[0, -1], [1, 0], [0, 1], [-1, 0]]
    if debugging:
        print ("wall corner", pos)
    ...
    seeDoor = False
    while True:#############
        if (doorStart == None) and lookingLeft (pos, leftVec[d], mapGrid, '. '):
            doorStart = addVec (pos, leftVec[d])
            doorEnd = doorStart
            #needToAvoidDoor = True
            seeDoor = True
        if lookingLeft (addVec (pos, forwardVec[d]), leftVec[d], mapGrid, '. '):
            if doorStart == None:
                doorStart = addVec (addVec (pos, forwardVec[d]), leftVec[d])
            doorEnd = addVec (addVec (pos, forwardVec[d]), leftVec[d])
            #needToAvoidDoor = True
            seeDoor = True
        else:
            if doorEnd != None:
                ...
                #needToAvoidDoor = True
                seeDoor = False
        if lookingLeft (addVec (pos, forwardVec[d]), leftVec[d], mapGrid, 'x '):
            if not seeDoor:#########
                lights, lightCount = checkLight(pos, lights, lightCount)
            else:
                lights, lightCount = checkLight (pos, lights, lightCount)
            #needToAvoidDoor = False
            pos = addVec (pos, forwardVec[d])
        elif lookingLeft (addVec (pos, forwardVec[d]), leftVec[d], mapGrid, 'x.'):
            doorStart = None
            doorEnd = None
            d = (d + 1) % 4 # turn right
            if s == pos: #if back at start
                return lights
        elif lookingLeft (addVec (pos, forwardVec[d]), leftVec[d], mapGrid, 'xx'):
            doorStart = None
            doorEnd = None
            d = (d + 1) % 4
            if s == pos: #if back at start
                return lights
        elif lookingLeft (addVec (pos, forwardVec[d]), leftVec[d], mapGrid, '  '):
            pos = addVec (pos, forwardVec[d])
            d = (d + 3) % 4 # turn left
            if s == pos: #if back at start
                return lights
        else:
            printf ("Scanning the room has gone wrong in IntroduceLights, maybe the room is too small")
...
def parseTextureDefault (room, x, y):
    ...
    elif expecting (['stairs']):
        expect ('stairs', room, x, y)
        rooms[room].defaultTexture['STAIRS'] = expectString (room, x, y, 'a texture after the stairs keyword')
...
def parseSpiralStairs (room, x, y):
    if expecting (['spiralstairs']):
        expect ('spiralstairs', room, x, y)
        expect ('height', room, x, y)
        h = expectInt (room, x, y, 'a height integer quantity after the height keyword')
        p = spiralstairs (x, y, h)
        rooms[room].spiralstairs += [p]
        return True
    return False
